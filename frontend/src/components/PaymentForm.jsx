...
const PaymentForm = ({ isOpen, onClose, onSubmit, payment, defaultDate }) => {
  ...
  useEffect(() => {
    if (payment) {
      ...
      setFormData((prev) => ({
        ...
        day_of_month: new Date(defaultDate).getDate(),
      }));
    }
  }, [payment, defaultDate, isOpen]);
  ...
  const showDayOfMonth = ['MONTHLY', 'QUARTERLY', 'ANNUAL'].includes(
    formData.recurrence
  );
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={payment ? 'Edit Payment' : 'Add New Payment'}
      size="lg"
    >
